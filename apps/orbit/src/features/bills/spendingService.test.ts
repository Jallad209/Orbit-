...
describe('spending log', () => {
  it('combines matching names and keeps a monthly summary reminder current', async () => {
    ...
    await createSpendingItem(
      repo,
      { name: 'Food', amount: 5, currency: 'JOD', spentOn: '2026-09-16' },
      clock,
    );
    await createSpendingItem(
      repo,
      { name: ' food ', amount: 15, currency: 'JOD', spentOn: '2026-09-17' },
      clock,
    );
    await createSpendingItem(
      repo,
      { name: 'Transport', amount: 3.5, currency: 'USD', spentOn: '2026-09-17' },
      clock,
    );

    const view = await loadSpending(repo, clock);
    expect(view.month.totals).toEqual([
      { currency: 'JOD', total: 20 },
      { currency: 'USD', total: 3.5 },
    ]);
    expect(view.month.groups).toEqual([
      { key: 'JOD:food', label: 'Food', currency: 'JOD', count: 2, total: 20 },
      { key: 'USD:transport', label: 'Transport', currency: 'USD', count: 1, total: 3.5 },
    ]);
    ...
    expect(reminder.body).toContain('20.00 JOD · 3.50 USD total');
  ...
  it('shows the previous month summary on the first day of a month', async () => {
    ...
    await createSpendingItem(
      repo,
      { name: 'Books', amount: 12, currency: 'JOD', spentOn: '2026-09-20' },
      september,
    );
    ...
    expect(view.previousMonth.totals).toEqual([{ currency: 'JOD', total: 12 }]);
